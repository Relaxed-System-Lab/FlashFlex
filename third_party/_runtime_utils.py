...
# A custom flag for tracking whether is running the first microbatch, if its' size is uneven
...
def _pre_forward(
    ...
    STAGE_LAYERS: int = HETERO_GROUPS['stage_idxs'][1] - HETERO_GROUPS['stage_idxs'][0]

    global FIRST_MICROBATCH, LAYER_COUNT
    state.training_state = TrainingState.FORWARD_BACKWARD
    
    state._exec_order_data.record_pre_forward(handles, module.training and FIRST_MICROBATCH)
    LAYER_COUNT += 1
    if LAYER_COUNT == STAGE_LAYERS:
